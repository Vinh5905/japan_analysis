...
```bash
make docker-push-release tag=latest
```

Nếu muốn version rõ ràng:

```bash
make docker-build-release tag=2026.07.29
make docker-push-release tag=2026.07.29
```
...
`release-up` sẽ start PostgreSQL và MinIO, chạy `minio-init`, sau đó chạy `crawler-init`. `crawler-init` làm hai việc:
