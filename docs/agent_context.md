...
- Trạng thái VPS OpenVPN đã cập nhật ngày 2026-09-16: file `vpn_config/vpngate_219.100.37.115_tcp_443.ovpn` đã được đưa vào `/etc/openvpn/client/japan.conf`. Config mới dùng remote `219.100.37.115:443`, certificate/key nhúng trong file và không có `auth-user-pass`. File trên VPS đã đặt quyền `600`; không ghi hoặc commit nội dung certificate/private key.
- Route giữ SSH đã được áp dụng trên VPS: SSH client hiện tại là `14.169.112.125`, default gateway là `103.249.116.1`, interface là `eth0`. Route runtime và dòng persistent trong `japan.conf` đều là `route 14.169.112.125 255.255.255.255 net_gateway`. IP này có thể thay đổi; agent phải lấy IP thật từ `$SSH_CLIENT` trước mỗi lần bật VPN, không dùng cứng IP cũ `118.69.133.159` trong ví dụ.
- Sau khi chỉnh route, `openvpn-client@japan` đang `enabled` nhưng `inactive`; VPN chưa được bật lại. Khi test phải đặt rollback `systemd-run --unit=openvpn-ssh-rollback --on-active=2min /bin/systemctl stop openvpn-client@japan`, start VPN, kiểm tra `ip route get <SSH_IP>`, rồi mới hủy rollback nếu SSH vẫn hoạt động.
